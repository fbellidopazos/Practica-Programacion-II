...
public class Mapa {
	...
	private void validar(PosicionXY pos) {
		if (!(pos.getX() >= 0 && pos.getX() <= maxCoordX) || !(pos.getY() >= 0 && pos.getY() <= maxCoordY))
			throw new IllegalArgumentException("Posici�n dada fuera del mapa:" + pos.getX() + ", " + pos.getY());		
	}
	...
	public void addObjetoEstatico(String cod, PosicionXY pos){
		...
		if(i < objetosEstaticos.size()){			
			throw new IllegalArgumentException("Ya hay en el mapa un objeto est�tico en la posici�n:" + 
					pos.getX() + " " + pos.getY());
		}
